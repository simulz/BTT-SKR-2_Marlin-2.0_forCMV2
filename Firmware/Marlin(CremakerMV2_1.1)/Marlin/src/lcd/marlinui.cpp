      ...
      if (encoderPastThreshold || lcd_clicked) {
        if (encoderPastThreshold && TERN1(IS_TFTGLCD_PANEL, !external_control)) {
          ...
          if (can_encode())
          {
            ...
            if(queue.is_lay1testing)
            {
              lay1Test.lay1EncoderPosition = encoderPosition;
            }
            if(boardTest.isKnobTesting)
            {
              boardTest.encorderPos = encoderPosition;
            }
          }
