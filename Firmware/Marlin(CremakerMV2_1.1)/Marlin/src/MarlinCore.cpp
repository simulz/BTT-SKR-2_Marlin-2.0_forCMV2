...
#include "feature/lay1test.h" //cremaker
#include "feature/loadcheckfilament.h" //cremaker
#include "feature/board_test.h" //cremaker
...
uint32_t old_encoderPosition = 0; //cremaker
...
uint32_t z_over_drop_ms;
uint32_t check_stepper_ms;
bool is_drop_testing = false;

void check_stepper_boardTest()
{
  if(!boardTest.isBoardTesting) return;

  if((millis() - check_stepper_ms) > 100)
  {
    if(boardTest.isWorkingAxisStepper[X_AXIS])
      boardTest.calAvgSGR(X_AXIS, stepperX.dir());

    if(boardTest.isWorkingAxisStepper[Y_AXIS])
      boardTest.calAvgSGR(Y_AXIS, !stepperY.dir());

    if(boardTest.isWorkingAxisStepper[Z_AXIS])
      boardTest.calAvgSGR(Z_AXIS, stepperZ.dir());

    if(boardTest.isWorkingAxisStepper[E_AXIS])
      boardTest.calAvgSGR(E_AXIS, stepperE0.dir());

    check_stepper_ms = millis();
  }
}

void check_knob_boardTest()
{
  if(!boardTest.isKnobTesting) return;

  if(boardTest.encorderPos != 0)
  {
    SERIAL_ECHO("KNOB T");
    SERIAL_ECHO(boardTest.encorderPos);
    boardTest.encorderPos = 0;
    SERIAL_EOL();
  }
}

// when extruder go down to z home and 
// can't trigger z for some time, stop 3d priner
void check_block_z_over_drop()
{
  if(!queue.is_z_home_testing) return;

  if(!queue.is_z_home_protect) return;
  
  //PORT_REDIRECT(SerialMask::All);
  //SERIAL_ECHO("SGR Z:");
  //SERIAL_ECHO(stepperZ.SG_RESULT());
  //SERIAL_EOL();

  if(stepperZ.SG_RESULT()<200)
  {
    if(!is_drop_testing)
    {
      z_over_drop_ms = millis();
      is_drop_testing = true;
    }
    else
    {
      if((millis()-z_over_drop_ms)>INTERVAL_Z_OVER_DROP)
      {
  ...
  {
    is_drop_testing = false;
    z_over_drop_ms = millis();
  }  
}

// while extruding filament, check load of stepper
void check_extrude_filament() 
{
  if(!loadcheckfilament.isExtrudeTesting) return;

  PORT_REDIRECT(SerialMask::All);
  SERIAL_ECHO("SGRESULT");
  SERIAL_ECHO(stepperE0.SG_RESULT());
  SERIAL_EOL();
} //cremaker

// In first layer test, change z-offset by rotating knob
void change_ZOffset_lay1test()
{
  if(!queue.is_lay1testing) return; // when it does not work lay1-testing, return 
  
  char offsetLZStr[30]="L offset Z";

  if((queue.ring_buffer.length < BUFSIZE) && !queue.is_printing_lay1) //&& lay1Test.numToLay1Test == LAY_1_TEST_START
  {
    lay1Test.lay1TestMove();
  }    

  if(lay1Test.lay1EncoderPosition != 0 && ((lay1Test.numToLay1Test != LAY_1_TEST_START) || queue.is_printing_lay1))
  {
    float offs;   

    if(lay1Test.lay1EncoderPosition > 2147483647L)
    {
      offs = 4294967295UL - lay1Test.lay1EncoderPosition; //  4294967295 - 4294967294 = 1 , 4294967295 is max value of unsinged long
      offs = (offs*-1) - 1 ; //0
    }
    else
    {
      offs = lay1Test.lay1EncoderPosition;      
    }
    ...
    PORT_REDIRECT(SerialMask::All);
    serialprintPGM(PSTR(offsetLZStr));
    SERIAL_ECHO(probe.offset.z);
    SERIAL_EOL();

    lay1Test.lay1EncoderPosition = 0;
...
void idle(TERN_(ADVANCED_PAUSE_FEATURE, bool no_stepper_sleep/*=false*/)) {
  ...
  change_ZOffset_lay1test(); //cremaker

  check_extrude_filament(); //cremaker

  check_block_z_over_drop(); //cremaker

  check_stepper_boardTest(); //cremaker

  check_knob_boardTest(); //cremaker
