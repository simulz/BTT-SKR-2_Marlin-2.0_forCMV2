...
public:
  ...
  FORCE_INLINE static bool valid_number(const char * const p) {
    ...
    static inline bool seen_weight(const char c){
      const uint8_t ind = LETTER_BIT(c);
      if (ind >= COUNT(param)) return false; // Only A-Z
      const bool b = TEST32(codebits, ind);
      if (b) {
          char * const ptr = command_ptr + param[ind] + 8;
          value_ptr = valid_number(ptr) ? ptr : nullptr;

          // SERIAL_ECHO("param[ind] : ");
          // SERIAL_ECHO(param[ind]);
          // SERIAL_ECHO("command_ptr : ");
          // SERIAL_ECHO(command_ptr);
          // SERIAL_ECHO("param is not true");
          // SERIAL_EOL(); 
      }
      return b;
    }
