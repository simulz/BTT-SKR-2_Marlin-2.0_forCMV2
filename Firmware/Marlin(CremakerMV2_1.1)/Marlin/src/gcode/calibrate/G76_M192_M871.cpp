...
void GcodeSuite::G76() {
  ...
  if (do_bed_cal) {
    ...
    celsius_t target_bed = cali_info_init[TSI_BED].start_temp,
            target_probe = temp_comp.isWinter ? temp_comp.bed_calib_probe_temp_winter : temp_comp.bed_calib_probe_temp;
  ...
  if (do_probe_cal) {
    ...
    TempSensorID tsi_index;

    if(temp_comp.isWinter)
    {
      tsi_index = TSI_PROBE_WINTER;
    }
    else
    {
      tsi_index = TSI_PROBE;
    ...
    celsius_t target_probe = cali_info_init[tsi_index].start_temp;
    ...
    for (;;) {
      ...
      const float measured_z = g76_probe(tsi_index, target_probe, noz_pos_xyz); 
      
      if (isnan(measured_z) || target_probe > cali_info_init[tsi_index].end_temp) break; 
    }

    SERIAL_ECHOLNPAIR("Retrieved measurements: ", temp_comp.get_index());
    if (temp_comp.finish_calibration(tsi_index))
      say_successfully_calibrated();
    else
      say_failed_to_calibrate();
...
void GcodeSuite::M871() {
  ...
  {
    uint16_t seasonVal = parser.value_linear_units();

    if(seasonVal == 0)
    {
      temp_comp.isWinter = false;
      SERIAL_ECHOLN("PTC_SUMMER");      
    }
    else if(seasonVal == 1)
    {
      temp_comp.isWinter = true;
      SERIAL_ECHOLN("PTC_WINTER");      
    }
    ...
    const TempSensorID mod = (parser.seen('B') ? TSI_BED :
                                ...
                                #endif
                                temp_comp.isWinter ? TSI_PROBE_WINTER :
                                TSI_PROBE
                              );
