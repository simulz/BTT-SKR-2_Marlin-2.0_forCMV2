...
void prepare_line_to_destination() {
   ...
   */
  void do_homing_move(const AxisEnum axis, const float distance, const feedRate_t fr_mm_s, const bool  ) {
  ...
  void homeaxis(const AxisEnum axis) {
    ...
    //
    if(axis == Z_AXIS) queue.is_z_home_testing = true; //cremaker
    const float move_length = 1.5f * max_length(TERN(DELTA, Z_AXIS, axis)) * axis_home_dir;
    if (DEBUGGING(LEVELING)) DEBUG_ECHOLNPAIR("Home Fast: ", move_length, "mm");
    do_homing_move(axis, move_length, 0.0, !use_probe_bump);
    if(axis == Z_AXIS) queue.is_z_home_testing = false; //cremaker
