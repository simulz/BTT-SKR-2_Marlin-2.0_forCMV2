...
typedef struct SettingsDataStruct {
  ...
  // Filament weight
  //
  float remain_filament_weight;
  float total_filament_weight;

  //
  // Filament density
  //
  float density_filament;
  ...
  // automatic printing right, left (cremaker)
  bool isEnableAutoPrint;
  bool rightOrLeft;
...
#include "../core/debug_out.h"
  ...
  bool MarlinSettings::save() {
    ...
    //
    {
      const float remain_filament_weight  = filWeight.filament_Remaining_Weight;
      const float total_filament_weight = filWeight.total_Used_Filament_Weight;
      EEPROM_WRITE(remain_filament_weight);
      EEPROM_WRITE(total_filament_weight);
    } //cremaker
    ...
    //
    {
      const float density_filament = filWeight.filament_Density;
      EEPROM_WRITE(density_filament);
    }//cremaker
    ...
    //
    {
      ...
      EEPROM_WRITE(bilinear_start);
        ...
        EEPROM_WRITE(added_z_value);         // cremaker
      #else
        dummyf = 0;
    ...
    //cremaker
    bool isEnableAutoPrint = automaticPrinting.isEnableAutoPrint;
    EEPROM_WRITE(isEnableAutoPrint);

    bool rightOrLeft = automaticPrinting.rightOrLeft;
    EEPROM_WRITE(rightOrLeft);
  ...
  bool MarlinSettings::_load() {
    ...
    if (strncmp(version, stored_ver, 3) != 0) {
      ...
      //
      {
        float remain_filament_weight;
        float total_filament_weight;
        EEPROM_READ(remain_filament_weight);
        EEPROM_READ(total_filament_weight);
        filWeight.setTUFilaWeight(total_filament_weight);
        filWeight.setFRWeight(remain_filament_weight);
      }//cremaker
      ...
      {
        float density_filament;
        EEPROM_READ(density_filament);
        filWeight.setFilaDensity(density_filament);
      }
      ...
      //
      {
        ...
        EEPROM_READ_ALWAYS(grid_max_y);                // 1 byte
          ...
          if (grid_max_x == (GRID_MAX_POINTS_X) && grid_max_y == (GRID_MAX_POINTS_Y)) {
            ...
            EEPROM_READ(added_z_value);                // cremaker
          }
      ...
      //cremaker
      bool isEnableAutoPrint;
      EEPROM_READ(isEnableAutoPrint);
      automaticPrinting.isEnableAutoPrint;

      bool rightOrLeft;
      EEPROM_READ(rightOrLeft);
      automaticPrinting.setRightOfLeft(rightOrLeft);
...
void MarlinSettings::reset() {
  ...
  filWeight.filament_Density = DEFAULT_FILAMENT_DENSITY;
