...
#define PTC_SAMPLE_END_WINTER ((PTC_SAMPLE_START_WINTER) + (PTC_SAMPLE_COUNT_WINTER) * (PTC_SAMPLE_RES_WINTER))

// Bed temperature calibration constants
#ifndef BTC_PROBE_TEMP
  #define BTC_PROBE_TEMP 30
#endif
#ifndef BTC_PROBE_TEMP_WINTER
...
static constexpr temp_calib_t cali_info_init[TSI_COUNT] = {
  ...
  #endif
  { PTC_SAMPLE_COUNT_WINTER, PTC_SAMPLE_RES_WINTER, PTC_SAMPLE_START_WINTER, PTC_SAMPLE_END_WINTER }, //Probe for winter
};
...
class ProbeTempComp {
  public:
    ...
    static constexpr celsius_t probe_calib_bed_temp        = BED_MAX_TARGET,         // Bed temperature while calibrating probe
                               ...
                               bed_calib_probe_temp_winter = BTC_PROBE_TEMP_WINTER;  // Probe temperature while calibrating bed for winter

    static int16_t *sensor_z_offsets[TSI_COUNT],
                   z_offsets_probe[cali_info_init[TSI_PROBE].measurements], // (µm)
                   z_offsets_bed[cali_info_init[TSI_BED].measurements],     // (µm)
                   z_offsets_probe_winter[cali_info_init[TSI_PROBE_WINTER].measurements];

    #if ENABLED(USE_TEMP_EXT_COMPENSATION)
      static int16_t z_offsets_ext[cali_info_init[TSI_EXT].measurements];   // (µm)
    #endif

    static bool isWinter; // false is summer, true is winter
